...
def main(user_input):
    ...
    prompt = f"""The user entered this input to define a routine of functions: {user_input}. Here is a list of the functions that are available to you: openApplication takes in one parameter, the name of the single application to open. closeApplication takes in one parameter, the name of the single application to close. speak takes in one parameter, the text to be spoken. sendMessage takes in two parameters, the phone number and message. takeScreenshot takes no parameters. weather also takes no parameters. Please output in JSON format the name of the routine and the LIST of functions that should be called with their parameters, using one function per application. For example, if the user says that whenever they say work mode to open slack and close their messages, please output {{"RoutineName": "work mode", "functions": [("openApplication", Slack), (closeApplication, Messages)]}}. Or if they said wake up mode should open safari and close messages, then output {{"RoutineName": "wake up mode", "functions": [("openApplication", Safari), (closeApplication, Messages)]}}."""
